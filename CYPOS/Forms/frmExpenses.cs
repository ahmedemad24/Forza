...
namespace cypos
{
    public partial class frmExpenses : Form
    {
        ...
        private void ExpenseFill(string strSearch,string strDateFrom,string strDateto)
        {
            ...
            if (txtSearch.Text == string.Empty)
            {
                strSQL = "SELECT tbl_Expense.id, tbl_Expense.expense_date, tbl_Expense.reference_no, tbl_ExpenseGroup.group_name,"+
                         ...
                         "WHERE tbl_Expense.expense_date  >= '" + strDateFrom + "' AND tbl_Expense.expense_date  <= '" + strDateto + "' order by tbl_Expense.expense_date DESC";
            }
            else
            {
                strSQL = "SELECT tbl_Expense.id, tbl_Expense.expense_date, tbl_Expense.reference_no, tbl_ExpenseGroup.group_name," +
                         ...
                         "AND tbl_Expense.expense_date  >= '" + strDateFrom + "' AND tbl_Expense.expense_date  <= '" + strDateto + "' order by tbl_Expense.expense_date DESC";
            }
            DataAccess.ExecuteSQL(strSQL);
        ...
        private void frmExpenses_Load(object sender, EventArgs e)
        {
            try
            {
                ExpenseFill("", new DateTime(2020,1,1).ToString("yyyy-MM-dd HH:mm:ss.fff"), DateTime.Now.Date.AddHours(23).AddMinutes(59).AddSeconds(59).AddMilliseconds(999).ToString("yyyy-MM-dd HH:mm:ss.fff"));
                FillCategory();
                dtpDate.Focus();
            }
            catch (Exception ex)
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            if (txtSearch.Text !=string.Empty)
            {
                ExpenseFill(txtSearch.Text, dtpFrom.Value.Date.ToString("yyyy-MM-dd HH:mm:ss.fff"), dtpTo.Value.Date.AddHours(23).AddMinutes(59).AddSeconds(59).AddMilliseconds(999).ToString("yyyy-MM-dd HH:mm:ss.fff"));
            }
        }
